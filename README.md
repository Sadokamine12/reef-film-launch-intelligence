...
`data/market_evidence.csv` now contains sourced public context for all preset markets: working-age population (20–64 proxy/context), public-transport time to ESO, provenance URLs, year/method notes, and explicit eligibility for using travel time in the scenario accessibility prior. Population is displayed as context only and does not increase ticket lift without calibration. Meta reachable audience, Google search demand, and ESO visitor-origin share remain blank until real platform/venue data is supplied; the app does not invent them.
